...
from model import lineConGraph
...
def loop(dataloader, model, optimizer, scheduler, device, args, mode='train'):
    ...
    with torch.set_grad_enabled(mode == 'train'):
        for step, batch in enumerate(tqdm(dataloader, desc=f"{mode.capitalize()} Progress", disable=False)):
            videoAudio,videoVisual,sentence_vectors, edge_idx_other, label, edge_idx_speaker, edge_idx_longdis,inter_modal_edges = batch
            output = model(sentence_vectors.squeeze(0),videoAudio.squeeze(0),videoVisual.squeeze(0).to(torch.float32), edge_idx_other.squeeze(0), edge_idx_speaker.squeeze(0), edge_idx_longdis.squeeze(0),inter_modal_edges.squeeze(0))
            # videoAudio, videoVisual, sentence_vectors, edge_idx_other, label, edge_idx_speaker, edge_idx_longdis = batch
            # output = model(sentence_vectors.squeeze(0),
            #                videoAudio.squeeze(0),
            #                videoVisual.squeeze(0).to(torch.float32),
            #                edge_idx_other.squeeze(0),
            #                edge_idx_speaker.squeeze(0),
            #                edge_idx_longdis.squeeze(0)
            #                )
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, device, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print(classification_report(val_epoch_labels, val_epoch_preds, target_names=args.target_names,digits=4))
